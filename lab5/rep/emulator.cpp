...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        std::cerr << "Порт не указан! Запустите эмулятор в таком виде: " << argv[0] << " <port_name>" << std::endl;
        return 1;
    }

    std::string portName = argv[1];
    std::cout << "Эмуляция на порту " << portName << "..." << std::endl;
    ...
    while (true) {
        ...
        if (serial.Write(msg) != cplib::SerialPort::RE_OK) {
            std::cerr << "Не получилось записать данные в порт" << std::endl;
        }
