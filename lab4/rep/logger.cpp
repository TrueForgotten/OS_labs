...
// Константы времени в секундах(сделал меньше для наглядности работы)
...
void clearAllLogs() {
    std::cout << "Очистка предыдущих логов" << std::endl;
...
void appendToLog(const std::string& filename, float value, time_t timestamp) {
    ...
    if (!outfile.is_open()) {
        std::cerr << "Невозможно сделать запись в лог " << filename << std::endl;
        return;
    }
            ...
            while ((pos = lineBuffer.find('\n')) != std::string::npos) {
                ...
                if (!line.empty() && line != "\r") {
                    try {
                        ...
                        std::cout << "Receiving: " << std::fixed << std::setprecision(2) << std::setw(6) << temp 
